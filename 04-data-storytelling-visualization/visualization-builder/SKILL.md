...
platforms:
  ...
  - linux
  - windows
description: Create effective, publication-ready data visualizations. Use when 
  choosing chart types, designing presentation visuals, building dashboard 
  charts, or applying visual design best practices to data output.
tags:
  - data-analytics
  - visualization
...
6. **Export and validate** — export at 300 DPI for print or 150 DPI for web. View the chart at the intended display size. Check: is the key message legible in under 5 seconds? Does it work in greyscale? **Post-generation visual verification**: (a) y-axis tick labels must NOT overlap — if ticks per axis-inch > 3 (e.g., 8 ticks on a 4-inch plot area), reduce tick count with `MaxNLocator(nbins=...)` or manual `set_yticks([])`. (b) Bar chart baseline must be appropriate for the data type — temperature data should not start at 0 (no meaningful zero baseline); counts/revenue should. (c) Verify Chinese/CJK font rendering renders correctly — missing glyphs show as boxes. (d) Check data annotations don't collide with axis labels or other annotations. See `references/axis_common_mistakes.md`. Complete `assets/viz_spec_template.md` if the chart is part of a larger deliverable.
